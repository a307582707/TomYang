...
| `k8s/addons/metrics-server/`（跳转 stub） | `k8s/addons/metrics-server/` | 目录命名 | B2 | 低 | stub 指向 `k8s/archived/metrics-server/`；归档目录是否同步改名见下 |
| `k8s/archived/metrics-server/` | `k8s/archived/metrics-server/` | 归档目录 | B2 | 中 | 与上游组件名 `metrics-server` 一致；更新 ARCHIVED/隔离脚本 |
| `k8s/addons/kube-dns/` | `k8s/addons/kube-dns/` | 大小写 | B2 | 中 | 建议同步迁入 `k8s/archived/kube-dns/`（EOL），本映射先定名 |
| `k8s/pki/controller-manager-csr.json` | `k8s/pki/controller-controller-manager-csr.json` | 文件名 | B3 | 中 | 需核对 cfssl 签发脚本/Wiki 是否硬编码旧名 |
...
| `k8s/ExtraAddons/prometheus/servicemonitor/kube-state-metrics-sm.yml` | `.../kube-state-metrics-sm.yml` | 文件名 | B3 | 低 | 与目录 `kube-state-metrics/` 对齐 |
| `k8s/ExtraAddons/prometheus/servicemonitor/prometheus-sm.yml` | `.../prometheus-sm.yml` | 扩展名 | B4 | 低 | 统一 `.yml`（本仓 102:2） |
| `k8s/archived/efk/fluentd-es-ds.yml` | `.../fluentd-es-ds.yml` | 扩展名 | B4 | 低 | 归档区一并统一 |
...
| Wiki 历史长文 | 含 `addons/metrics-server`、`ExtraAddons/dashboard` 的 **归档正文**可不改路径（仅教材）；操作页必须更新 |
