# 不是靠「删机器」：把 Request 虚高和闲置命名空间算清楚之后，成本降了 38%
...
结论：HPA 放大的是 Request 的错误；Request 不准时，自动化会加速烧钱。
...
我们把 CPU Request 从「申请制」改成「观测校准制」，在 90 天零相关事故前提下，节点费用下降 38%。关键不是更便宜的云厂商，而是更诚实的容量数据。
