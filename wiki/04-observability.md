# 可观测性落地：一次 2.3 秒慢请求，我们怎么从网关追到一行 SQL

> 题材类型：可观测性（排查路径，不是工具安装）  
> 适合标题：`别再只贴 Grafana 截图了：慢请求的标准追踪剧本`  
> 目标：展示你有「闭环能力」——指标发现问题，链路定位位置，日志给出证据
...
| 价值 | 用同一套「RED → Trace → Log → Profile/SQL」方法，41 分钟定位并止血 |
...
工具可以是 Prometheus + Tempo/Jaeger + Loki + Pyroscope，**文章重点是路径，不是品牌**。

---

## 2. 排查剧本（逐步，可当 Runbook）
...
> 指标负责叫醒你，链路负责带路，日志负责呈堂证供。  
> 三样缺一样，oncall 就只能猜。

把本案例做成团队训练题：给一份脱敏瀑布图 + 日志，让新人在 30 分钟内写出根因假设。
